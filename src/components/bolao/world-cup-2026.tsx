...
export function WorldCup2026Banner({ compact }: { compact?: boolean }) {
  ...
  return (
    <div
      className={`wc-2026-banner relative overflow-hidden rounded-3xl ${compact ? "p-4" : "p-5"}`}
    >
      <div className="relative flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className={`font-display font-bold leading-tight ${compact ? "text-lg" : "text-xl"}`}>
            {wc.title}
          </h2>
  ...
  return (
    <button
      ...
      className={`rounded-2xl p-3.5 text-left transition border active:scale-[0.98] ${
        selected
          ? "border-transparent wc-stage-selected"
          : "glass border-border hover:bg-surface-2/60"
      }`}
    >
      <div className="text-2xl leading-none">{icon}</div>
      <div className="text-sm font-bold mt-2 leading-tight">{label}</div>
      <div
        className={`text-[11px] mt-0.5 leading-snug ${selected ? "opacity-90" : "text-foreground/70"}`}
      >
        {desc}
      </div>
    </button>
