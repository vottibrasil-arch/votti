...
Não use outro projeto Supabase (ex.: legado Palpite Gol). O app bloqueia cadastro se detectar projeto diferente.
...
Saída esperada: `✅ Supabase OK — tabela polls acessível.`
...
Valores oficiais do VOTTI (projeto `ppvhlocqetyrsqidijms`):

```
VITE_SUPABASE_URL=https://ppvhlocqetyrsqidijms.supabase.co
VITE_SUPABASE_ANON_KEY=sb_publishable_…  (Publishable key do painel → API)
```

> Use a URL **sem** `/rest/v1/`. O app corrige automaticamente URL/chave antigas do Lovable.
...
Se usar **Lovable**, atualize também as variáveis de ambiente no painel do Lovable — elas podem sobrescrever o `.env` local.
